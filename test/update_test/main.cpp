...
class ObjectInfoComponent : public Component {
public:
    ObjectInfoComponent():Component(CustomType){}
    // Updates each component before every frame
    void Update() {
        cout << "deltaTime: " << FrameTime::GetDeltaTime() 
                << " Time " << FrameTime::GetTime() 
                << " Frame count " << FrameTime::GetFrame() << endl;
    }
};
...
void initWorld(const char *filename) {
    ...
    // Add Update info component to first object
    ObjectInfoComponent *oic = new ObjectInfoComponent();
    SceneObject *firstSceneObject = (*(renderBase->GetSceneObjects()))[0];
    firstSceneObject->AddCompnent(oic);
}
